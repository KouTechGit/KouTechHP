...
- **`youtube_id`**: YouTube動画ID（URLから抽出）
  - `null`の場合は「準備中」として表示
- **`duration`**: 動画の再生時間（現在は"00:00"で統一）
- **`log_link`**: 対応する授業ログへのリンク（現在は`null`で統一）
